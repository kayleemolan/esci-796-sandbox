...
#%% Convert datetime column to dates, then set as index -- Andrew
data['datetime'] = pd.to_datetime(data['datetime'], format='%Y-%m-%d %H:%M')
data.set_index(['datetime'], inplace = True)

#%% Wrangle discharge column -- Brian    
...
#%% Find first 10/1 and last 9/30 in record -- Jeremy
...
#%% Trim data to whole water years -- Kaylee
data = data[startdate:enddate]   # contains discharge data in cfs 

#%% Initialize empty data frame for results -- Kevin
...
#%% Average seasonal record over all years -- Nick
for freq in freq_vec:
    dfcum[freq] = data.groupby(data.index.dayofyear).quantile(freq)

#%% Create bonus rows containing NaN's in between each year -- Madison
...
#%% Add bonus rows to data series (to avoid plotting overlap) -- Nora
data = pd.concat([data,newvalues])
data.sort_index(inplace = True)

#%% Plot time series of seasonal discharge -- Jon-Erik
